...
namespace Functional.DotNet
{
    ...
    public static class TaskExt
    {
        ...
        public static Either<TResult, Error> RunSync<TResult>(Func<Task<TResult>?> task)
        {
            var outcomeTask = Task.Run(task);
            outcomeTask.Wait();
            var outcome = outcomeTask.Map<TResult, Either<TResult, Error>>(
                     Faulted: (ex) => Error(ex.Message),
                     Completed: (item) => Left(item));
            return outcome.Result;
        }


        public static Either<IEnumerable<TResult>, Error> RunSync<TResult>(Func<Task<IEnumerable<TResult>>?> task)
        {
            var outcomeTask = Task.Run(task);
            outcomeTask.Wait();
            var outcome = outcomeTask.Map<IEnumerable<TResult>, Either<IEnumerable<TResult>, Error>>(
                     Faulted: (ex) => Error(ex.Message),
                     Completed: (item) => Left(item));
            return outcome.Result;
        }
